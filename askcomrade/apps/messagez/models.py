...
class Message(models.Model):
    ...
    body = models.ForeignKey(MessageBody, related_name='messagez', verbose_name=_("Message"))
    ...
    def inbox_count_for(user):
        "Returns the number of unread messagez for the given user but does not mark them seen"
...
class MessageBodyAdmin(admin.ModelAdmin):
    ...
    list_select_related = ["author","parent_msg"]

# Admin interface to MessageBody
class MessageAdmin(admin.ModelAdmin):
    search_fields = ('user',)
    list_select_related = ["user", "body"]
