...
from askcomrade.apps.posts.models import Tag
...
from askcomrade.server.views import LATEST

register = template.Library()


@register.simple_tag
def get_cuont(counts, word):
    try:
...
def gravatar(user, size=80):
    ...
    if user.is_suspended:
        ...
        email = 'suspended@askcomrades.org'
    else:
        email = user.email
    hash = hashlib.md5(email.encode('utf-8')).hexdigest(),
    # hash = hashlib.sha256(str(random.getrandbits(email)).encode('utf-8')).hexdigest()
...
def post_body(context, post, user, tree):
    ...
    return {'post': post, 'user': user, 'tree': tree, 'request': context['request']}
    #isaac return dict(post=post, user=user, tree=tree, request=context['request'])
...
COMMENT_BODY = get_template(COMMENT_TEMPLATE)
...
#
#     def traverse(node):
#         data = '<div class="indent"> hello word</div>'
...
#     coll = '<h2>helo <button class="btn btn-success"> Button</button></h2>'
#     for node in tree[post.id]:
#         coll.join(traverse(node))
#         return (coll)
#     return mark_safe(coll)
